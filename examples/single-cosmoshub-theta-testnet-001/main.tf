...
module "cosmoshub_thetatestnet001" {
  source = "../../"
  ...
  node_source          = "https://github.com/cosmos/gaia.git"
  node_binary          = "gaiad"
  node_dir             = "~/.gaia"
  ...
  node_snapshot_code = <<EOF
      gaid tendermint unsafe-reset-all
      LATEST=$(curl -s https://snapshots2.polkachu.com/snapshots/ | grep -oE 'cosmos/cosmos_.*.tar.lz4' | cut -f 1 -d '<' | head -1)
  ...
  extra_commands = <<EOF
    ...
    dasel put string -f $DAEMON_HOME/config/app.toml -p toml "minimum-gas-prices" 0.025uatom
    ...
    dasel put string -f $DAEMON_HOME/config/config.toml -p toml ".p2p.seeds" 639d50339d7045436c756a042906b9a69970913f@seed-01.theta-testnet.polypore.xyz:26656,3e506472683ceb7ed75c1578d092c79785c27857@seed-02.theta-testnet.polypore.xyz:26656

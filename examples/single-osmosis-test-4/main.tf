...
module "osmosis_test4" {
  ...
  node_snapshot_code = <<EOF
      ...
      EOF
  dasel_commands     = <<EOF
    dasel put string -f app.toml -p toml "pruning" custom
    dasel put string -f app.toml -p toml "pruning-keep-recent" 100
    dasel put string -f app.toml -p toml "pruning-keep-every" 0
    dasel put string -f app.toml -p toml "pruning-interval" 10
    dasel put string -f app.toml -p toml ".api.enable" true
    dasel put string -f app.toml -p toml ".api.address" tcp://0.0.0.0:1318
    dasel put string -f app.toml -p toml ".api.swagger" true
    dasel put string -f app.toml -p toml ".grpc.enable" true
    dasel put string -f app.toml -p toml ".grpc.address" 0.0.0.0:9090

    dasel put string -f config.toml -p toml ".rpc.laddr" tcp://0.0.0.0:26657
    dasel put string -f config.toml -p toml ".p2p.pex" true
    dasel put string -f config.toml -p toml ".p2p.laddr" tcp://0.0.0.0:26656
    dasel put string -f config.toml -p toml ".p2p.seeds" 0f9a9c694c46bd28ad9ad6126e923993fc6c56b1@137.184.181.105:26656
    dasel put string -f config.toml -p toml ".p2p.persistent_peers" 4ab030b7fd75ed895c48bcc899b99c17a396736b@137.184.190.127:26656,3dbffa30baab16cc8597df02945dcee0aa0a4581@143.198.139.33:26656
    dasel put string -f config.toml -p toml ".p2p.addr_book_strict" false
    dasel put string -f config.toml -p toml ".p2p.max_num_inbound_peers" 80
    dasel put string -f config.toml -p toml ".p2p.max_num_outbound_peers" 80
    dasel put string -f config.toml -p toml ".instrumentation.prometheus" true

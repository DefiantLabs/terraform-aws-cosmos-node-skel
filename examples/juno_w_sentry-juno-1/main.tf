...
module "sentry" {
  ...
  node_snapshot_code = <<EOF
      ...
      aria2c -d $DAEMON_HOME https://snapshots2.polkachu.com/snapshots/$LATEST
  ...
  extra_commands = <<EOF
    ...
    dasel put string -f $DAEMON_HOME/config/config.toml -p toml ".p2p.persistent_peers" validatornode_:26656,ddaa333eba3d8ac43249dcd59451aad28f190d70@162.55.128.249:36656,f79ce2fab55e56b408d76ddcbc1c82c1a90e315b@172.31.44.248:26656
    dasel put string -f $DAEMON_HOME/config/config.toml -p toml ".p2p.private_peer_ids" validatornode_:26656
...
module "validator" {
  ...
  extra_commands = <<EOF
    ...
    dasel put string -f $DAEMON_HOME/config/app.toml -p toml ".api.swagger" true
    dasel put string -f $DAEMON_HOME/config/app.toml -p toml ".grpc.enable" true
    dasel put string -f $DAEMON_HOME/config/app.toml -p toml ".grpc.address" 127.0.0.1:9090


    dasel put string -f $DAEMON_HOME/config/config.toml -p toml "moniker" Defiant
    dasel put string -f $DAEMON_HOME/config/config.toml -p toml ".rpc.laddr" tcp://127.0.0.1:26657
        dasel put string -f $DAEMON_HOME/config/config.toml -p toml ".p2p.pex" false
    dasel put string -f $DAEMON_HOME/config/config.toml -p toml ".p2p.laddr" tcp://0.0.0.0:26656

    dasel put string -f $DAEMON_HOME/config/config.toml -p toml ".p2p.persistent_peers" change_to_sentry
    dasel put string -f $DAEMON_HOME/config/config.toml -p toml ".p2p.addr_book_strict" false
    dasel put string -f $DAEMON_HOME/config/config.toml -p toml ".instrumentation.prometheus" true
